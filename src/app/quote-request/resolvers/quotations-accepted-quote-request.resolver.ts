...
@Injectable()
export class QuotationsAcceptedQuoteRequestResolver implements Resolve<boolean | Quotation[]> {
  ...
  resolve(route: ActivatedRouteSnapshot): Observable<boolean | Quotation[]> {
    return this.quotationService.getAcceptedPerQuoteRequest(route.params['id']).pipe(
      catchError(err => {
        console.error(err);
