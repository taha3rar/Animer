...
class FormConfig {
  ...
  minLength: number = 0;
  maxLength: number = 0;
  constructor(v: string[], type: string) {
    this.validators = v;
    this.type = type;
  }
}
...
export class FormListComponent extends BaseValidationComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private dn: DynamicFormsService) {
    ...
    this.configForm.valueChanges.subscribe((data) => {
      if (data) {
        ...
        this.currentConfig["type"] = data.type;
        this.currentConfig["label"] = data.label;
        this.currentConfig["placeholder"] = data.placeholder;
        this.currentConfig["tooltip"] = data.tooltip;
        this.currentConfig["req"] = data.req;
        this.currentConfig["validators"] = data.validators;
  ...
  drop(event: any) {
    if (event.previousContainer === event.container) {
      ...
      const type = event.previousContainer.data[event.previousIndex].title;
      const p = new FormConfig([], type);
  ...
  add(a: any, e: string) {
    ...
    const type = e;
    console.log(e);
    let config = new FormConfig([], type);
    console.log(config);
  ...
  changeConfig(title: any) {
    if (title.config == this.currentConfig) {
      console.log("yes");
    } else {
      this.currentConfig = title.config;
  ...
  validatorChange(e: { value: string }) {
    if (e.value.includes("Min Value")) {
      this.configForm.get("min").setValidators([Validators.required, Validators.min(0)]);
    ...
    if (e.value.includes("Max Value")) {
      this.configForm.get("max").setValidators([Validators.required, Validators.min(1)]);
    ...
    if (e.value.includes("Min Length")) {
      this.configForm.get("minLength").setValidators([Validators.required, Validators.min(0)]);
    ...
    if (e.value.includes("Max Length")) {
      this.configForm.get("maxLength").setValidators([Validators.required, Validators.min(1)]);
  ...
  isFieldInvalid2(field: string, form: FormGroup) {
    const control = form.get(field);
    if (control.invalid && control.touched) {
      if (control.hasError("minLength")) {
        return { "min-length": "Minimum length error" };
      } else if (control.hasError("maxLength")) {
        return { "max-length": "Maximum length error" };
      } else if (control.hasError("min")) {
        return { "min-value": "Minimum value error" };
      } else if (control.hasError("max")) {
        return { "max-value": "Maximum value error" };
  ...
  showFieldStyle2(field: string, form: FormGroup) {
    if (!this.isFieldInvalid2(field, form)) {
      return undefined;
    }
    return {
      [Object.keys(this.isFieldInvalid2(field, form))[0]]: this.isFieldInvalid2(field, form),
    };
  }
  errorMessage(field: string, form: FormGroup) {
    return Object.values(this.isFieldInvalid2(field, form))[0];
  }
  removeField(input: any, i: number) {
    if (input.titles.length > 1) {
      input.titles.splice(i, 1);
    }
  }
  submitConfigs() {
    console.log(this.inputs);
    const dynamicForm: DynamicForm = {} as DynamicForm;
    dynamicForm.inputs = [];
    dynamicForm.formRows = this.inputs.length;
    for (let i = 0; i < this.inputs.length; i++) {
      const rowNumber = i + 1;
      const input: any = this.inputs[i];
      input.titles.forEach((data: any) => {
        let dFormInput: DynamicFormInput = {
          fieldName: data.id,
          label: data.config.label,
          validators: this.getValidator(data.config),
          placeholder: data.config.placeholder,
          required: data.config.req,
          color: data.config.color,
          rowNumber: rowNumber,
          size: 12 / input.titles.length,
          type: data.config.type.toLowerCase(),
          tooltip: data.config.tooltip,
        };
        dynamicForm.inputs.push(dFormInput);
      });
    }
  ...
  getValidator(config: any) {
    const validators = [];
    if (config.validators.includes("Min Value")) {
      validators.push(Validators.min(config.min));
    }
    if (config.validators.includes("Min Length")) {
      validators.push(Validators.min(config.minLength));
    }
    if (config.validators.includes("Max Value")) {
      validators.push(Validators.min(config.max));
    }
    if (config.validators.includes("Max Length")) {
      validators.push(Validators.min(config.maxLength));
    }
    if (config.req) {
